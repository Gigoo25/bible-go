...
An optional `~/.config/bible-go/colors.json` overrides the colors last, over both
the theme and colors set in `config.json`. bible-go never writes it, so it can be
generated by a dotfiles setup from the desktop palette. Each field is optional and
must be `#rrggbb`; missing or malformed ones are ignored:
```json
{
  "highlightColor": "#e5c799",
  "verseNumColor": "#cbaba0",
  "textColor": "#dcd8cd",
  "dimColor": "#3a3028"
}
```
